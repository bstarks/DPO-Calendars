...
(function (id0, undefined) {
	...
	function buildCalendarHtmlForYear(yr) {
		...
		for (var m = 0; m < 12; m++) { // For each month
			if (m === 0) months[m] = '<tr>' +
				...
				'<td class="wcal_month">' + month[m] + '</td>';
			limit = daysInMonth[m];
			for (var d = 0; d < 31; d++) { // For each day in current month array
				if (d < limit) {
					xo = xoPattern[pIndex][0];
					day = xoPattern[pIndex][1];
					days[d] = '<td class="wcal_' + xo + '"><div class="wcal_xo"><div><img alt="" src="' + (xo === "x" ? img_x : img_o) + '" /></div><div class="wcal_con">' + day + '</div></div></td>';
					pIndex++;
				} else {
					days[d] = '<td class="wcal_blank"><div class="wcal_xo"><div><img alt="" src="' + img_blank + '" /></div></div></td>';
				}
			}
			months[m] += days.join('') + '</tr>';
		}
		return months.join('');
	}
	...
	function $() {
		...
		if (arguments.length === 1 && isArray(arguments[0])) elements = arguments[0];
		else for (i = 0, l = arguments.length; i < l; i++)	elements.push(arguments[i]);
		for (i = 0, l = elements.length; i < l; i++) {
			element = elements[i];
			if (typeof element === 'string') element = document.getElementById(element);
			//if (elements.length === 1) return element;
			elements.push(element);
		}
		return this;
	}
	
	$.prototype.classList = function() {
		var elements = this.elements,
			classList = [];
		for (var i = 0, l = elements.length; i < l; i++) classList[i] = elements[i].getAttribute('class');
		return this;
	}
	...
	$.classList.prototype.has = function (className) {
		...
		for (var i = 0, l = classList.length; i < l; i++) if (classList[i] === className) return true;
		return false;
	}*/
	...
	function classList(el, action, className) {
		...
		function add() {
			if (className && !has()) {
				classNames = classNames.push(className).join(' ');
		...
		function toggle() {
			if (!className) el.style.display = (el.style.display !== 'block' ? 'block' : 'none');
			else {
				if (has()) remove();
				else add();
			}
	...
	c.tb = { // These are the functions called from the toolbar
		...
		"help": function (whichCal) { classList('wcal_helpBox', 'toggle'); },
	...
	function main() {
		var applyToElements = function (elName, fn) {
				var els = document.querySelectorAll(elName);
				for (var i = 0, l = els.length; i < l; i++) fn(els[i]);
			};
		
		// Set images for boxes based on their class
		applyToElements('.wcal_img_x', function(el) { el.src = cfg.img_src_x; });
		applyToElements('.wcal_img_o', function(el) { el.src = cfg.img_src_o; });
		...
		applyToElements('.wcal_view', function (el) { el.setAttribute("checked", "checked"); });
		document.getElementById('wcal_view1').setAttribute("checked", "checked");
		document.getElementById('wcal_view2').setAttribute("checked", "checked");
		...
		applyToElements('.wcal_view', function (el) { el.onchange = function () { id0.wcal.newView(el); }; });
		// Ensure View1 (calendar) is disabled so it will always be displayed
		document.getElementById('wcal_view1').disabled = 'disabled';
		...
		applyToElements('.wcal_dropdown_arrow', function (el) { // fix!!!
			el.onmouseover = classList(el, 'add', 'wcal_active');
			el.onmouseout = classList(el, 'remove', 'wcal_active');
		});
		document.getElementById('wcal_dropdown_arrow').onmouseover = function () { classList('wcal_dropdown_arrow', 'toggle', 'wcal_active') }
	}
